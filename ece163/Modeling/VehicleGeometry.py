...
This file contains function to update new points in ENU standard orientation.
"""
...
class VehicleGeometry():
	...
	def getNewPoints(self, x, y, z, yaw, pitch, roll):
		...
		pos = [[x,y,z]]#format displacement in way we can manipulate
		dcm = MatrixMath.transpose(Rotations.euler2DCM(yaw, pitch, roll))#inverse DCM to convert body frame to inertial
		newPoints = MatrixMath.transpose(MatrixMath.multiply(dcm, MatrixMath.transpose(newPoints)))#transform vehicle points from body to inertial
		newPoints = Rotations.ned2enu(newPoints)#change from NED to ENU
		newpos = Rotations.ned2enu(pos)#change displacement from NED to ENU
		newPoints = MatrixMath.offset(newPoints, newpos[0][0], newpos[0][1], newpos[0][2])#add displacement to current vehicle points
